...
class SignUpSchoolAdmin(admin.ModelAdmin):
    list_display = ['id', 'name', 'select', 'is_active', 'phone', 'viloyat', 'region', 'district', 'tuman', 'text',
                    'pub_date']
...
@admin.register(Nabor)
class NaborAdmin(admin.ModelAdmin):
    list_display = ['id', 'name', 'region', 'district', 'is_start', 'category', 'pub_date']
    list_display_links = ['name']
    list_filter = ['is_start', 'region', 'district', 'category']
    save_on_top = True
